...
function ConfigAuxiliar() {
    ...
    if (newImageUrl && userId) {
      try {
        ...
        if (response.status == 200) {
          ...
          sessionStorage.getItem('userImage', newImageUrl);
          setnewImageUrl('');
  ...
  return (
    ...
    <div className="container_perfil">
      ...
      <div className="box_center">
        <img className='imagenConfig p_a' src={imageUrl} alt="" />
      ...
      <div className="container_subir_img box_center text-center">
        ...
        <div className="input_subir_img">
          <input type="text" className='form-control input_url text-center' value={newImageUrl} onChange={handleInputChange} />
        </div>

        <div className="boton_subir_img">
          <center>
            <Button className='boton_add_img' label="Cargar"  icon="pi pi-check" onClick={handleUpload}/>
          </center>
        </div>

        <div className="boton_subir_img">
          <label className='text_paragraphs'>CONVERTIR IMAGEN A URL</label>
          <NavLink to='/convertir-imagen-auxiliar'>
